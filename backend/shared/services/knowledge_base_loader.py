...
class KnowledgeBaseLoader:
    ...
    def load_chunks(self, contract_type: str) -> Optional[list]:
        """
        청크 메타데이터 로드 (FAISS 인덱스에서 임베딩 포함)
        
        Args:
            contract_type: 계약 유형
            
        Returns:
            청크 리스트 (임베딩 포함) 또는 None
        """
        # 캐시 확인
        ...
        try:
            ...
            # FAISS 인덱스에서 임베딩 로드 및 청크에 추가
            faiss_index = self.load_faiss_index(contract_type)
            
            if faiss_index is not None and faiss_index.ntotal == len(chunks):
                logger.info(f"FAISS 인덱스에서 임베딩 추가 중: {contract_type}")
                
                # 각 청크에 임베딩 추가
                for i, chunk in enumerate(chunks):
                    try:
                        # FAISS 인덱스에서 i번째 벡터 추출
                        embedding_vector = faiss_index.reconstruct(int(i))
                        chunk["embedding"] = embedding_vector.tolist()
                    except Exception as e:
                        logger.warning(f"청크 {i} 임베딩 추출 실패: {e}")
                        chunk["embedding"] = None
                
                logger.info(f"✅ 임베딩 추가 완료: {len(chunks)}개 청크")
            else:
                if faiss_index is None:
                    logger.warning(f"FAISS 인덱스를 찾을 수 없음: {contract_type} (임베딩 없이 반환)")
                else:
                    logger.warning(f"FAISS 인덱스 크기 불일치: {faiss_index.ntotal} vs {len(chunks)} (임베딩 없이 반환)")
