...
2. **무관** – 의미적으로 관련 없음
3. 반드시 Top-3 후보 모두에 대해 판단하고, 각 후보의 실제 조문 내용을 1~2줄로 인용한 뒤 분석하세요.
4. 각 후보의 판단에는 포함/누락된 핵심 요소와, 그로 인한 잠재적 리스크(법적·운영적)를 설명할 것.
5. confidence: 0.0~1.0  
   (0.6 이상 = 의미 유사, 0.3~0.6 = 부분 유사/표현 차이형, 0.3 미만 = 무관)

---

### **작성 가이드**
각 후보는 다음 항목으로 구성하며, 자연스러운 문단으로 서술합니다.

1. **reasoning**  
   - 후보 조문의 핵심 문장을 「 」로 인용하거나 1~2줄로 요약 인용.  
   - 표준 조문의 핵심 요소(의무, 조건, 절차 등)를 기준으로 포함/누락 관계를 설명.  
   - 문단형 서술 (약 100~200자).

2. **risk**  
   - "이 조항이 없으면..." 형식으로 시작하여, 계약 체결·이행·분쟁 시 발생할 수 있는 구체적 문제를 설명.  
   - 문단형 서술 (약 80~150자).

3. **recommendation**  
   - 구체적 보완 방안을 제시하고, 문장을 “~을 추가할 것을 권장합니다.”로 마무리.  
   - 문단형 서술 (약 80~150자).

4. **summary (전체 요약)**  
   - Top-3 후보를 종합적으로 검토한 결과를 문단으로 서술 (3~5문장).  
   - 왜 이 표준 조문이 누락으로 판단되었는지 자연스럽게 설명.  

5. **overall_risk (전체 위험)**  
   - "이 조항이 없으면..."으로 시작하여 계약 전체 관점의 리스크를 시나리오형으로 기술 (2~3문장).
...
{{
  "candidates": [
    {{
      ...
      "reasoning": "후보 조문 내용을 인용한 뒤, 표준 조문과의 포함/누락 관계를 문단으로 설명 (100~200자)",
      "risk": "이 조항이 없으면 ... (구체적 시나리오) (80~150자)",
      "recommendation": "보완 방안을 제시하고 '~을 추가할 것을 권장합니다.'로 마무리 (80~150자)"
    }},
    ...
  ],
  "summary": "Top-3 후보를 종합적으로 검토한 결과, 표준 조문이 누락된 이유를 문단으로 서술 (3~5문장).",
  "overall_risk": "이 조항이 없을 때의 전체 리스크를 시나리오 형태로 기술 (2~3문장)."
}}
