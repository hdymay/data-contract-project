...
**작성 가이드:**
1. **근거(reasoning)**: 표준 조문의 핵심 요소(의무, 조건, 절차 등)를 파악하고, 각 후보가 이를 얼마나 포함하는지 자연스럽게 서술하세요. 누락된 부분이 있다면 구체적으로 명시하세요. (2-3문장의 연결된 문단)

2. **위험(risk)**: "이 조항이 없으면..." 형식의 시나리오로 작성하세요. 계약 체결·이행·분쟁 시 발생할 수 있는 구체적인 문제 상황을 설명하세요. (1-2문장의 자연스러운 서술)

3. **종합 분석(summary)**: Top-3 후보를 종합적으로 검토한 결과를 문단으로 작성하세요. 왜 이 표준 조문이 누락으로 판단되었는지 자연스럽게 설명하세요. (3-5문장의 연결된 문단)
...
{{
    "candidates": [
        {{
            ...
            "reasoning": "표준 조문의 핵심 요소와 후보 조문의 비교를 자연스러운 문단으로 서술 (2-3문장)",
    ...
    "summary": "Top-3 후보를 종합적으로 검토한 결과를 자연스러운 문단으로 서술. 왜 이 표준 조문이 누락으로 판단되었는지 설명 (3-5문장의 연결된 문단)",
