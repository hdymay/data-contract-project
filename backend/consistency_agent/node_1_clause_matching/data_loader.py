...
class ContractDataLoader:
    ...
    def load_standard_contract(
        self, 
        file_path: Optional[Path] = None,
        contract_type: str = "provide",
        use_knowledge_base: bool = True
    ) -> List[ClauseData]:
        """
        표준 계약서를 로드 (KnowledgeBaseLoader 또는 직접 파일 로드)
        
        Args:
            file_path: JSONL 파일 경로 (use_knowledge_base=False일 때 사용)
            contract_type: 계약 유형 (provide, create, process, brokerage_provider, brokerage_user)
            use_knowledge_base: True면 KnowledgeBaseLoader 사용, False면 직접 파일 로드
        ...
        """
        # KnowledgeBaseLoader 사용 (권장)
        if use_knowledge_base:
            try:
                from backend.shared.services import get_knowledge_base_loader
                
                self.logger.info(f"KnowledgeBaseLoader로 표준 계약서 로드 중: {contract_type}")
                
                kb_loader = get_knowledge_base_loader()
                chunks_data = kb_loader.load_chunks(contract_type)
                
                if not chunks_data:
                    raise FileNotFoundError(f"표준 계약서 청크를 찾을 수 없습니다: {contract_type}")
                
                # 청크 데이터를 ClauseData로 변환
                clauses = []
                for chunk in chunks_data:
                    try:
                        clause = self._parse_clause_from_dict(chunk)
                        clauses.append(clause)
                    except ValueError as e:
                        self.logger.warning(f"데이터 검증 오류 (ID: {chunk.get('id', 'unknown')}): {e}")
                        continue
                
                self.logger.info(f"✅ KnowledgeBaseLoader로 {len(clauses)}개 조문 로드 완료 (임베딩 포함)")
                return clauses
                
            except Exception as e:
                self.logger.warning(f"KnowledgeBaseLoader 사용 실패, 직접 파일 로드로 전환: {e}")
        ...
        self.logger.info(f"표준 계약서 직접 로드 중: {file_path}")
    ...
    def merge_exhibit_references(
        self,
        clauses: List[ClauseData]
    ) -> List[ClauseData]:
        """
        조문에 별지 참조([별지n])가 있으면 해당 별지 내용을 병합
        
        Args:
            clauses: ClauseData 객체 리스트
        
        Returns:
            별지 내용이 병합된 ClauseData 객체 리스트
        """
        import re
        
        # 별지 데이터를 ID로 매핑 (별지1, 별지2, 별지3, 별지4)
        exhibit_map = {}
        for clause in clauses:
            # 별지 ID 패턴: "별지1-1", "별지2", "별지3", "별지4" 등
            if clause.id and re.match(r'별지\d+', clause.id):
                # 별지 번호 추출 (예: "별지1-1" -> "별지1")
                exhibit_num = re.match(r'(별지\d+)', clause.id).group(1)
                if exhibit_num not in exhibit_map:
                    exhibit_map[exhibit_num] = []
                exhibit_map[exhibit_num].append(clause)
        
        self.logger.info(f"발견된 별지: {list(exhibit_map.keys())}")
        ...
        for clause in clauses:
            ...
            # 조문에서 [별지n] 참조 찾기
            exhibit_refs = re.findall(r'\[별지(\d+)\]', clause.text)
            
            if exhibit_refs:
                # 별지 내용 수집
                exhibit_contents_raw = []
                exhibit_contents_norm = []
                
                for ref_num in set(exhibit_refs):  # 중복 제거
                ...
                # 별지 내용이 있으면 병합
                if exhibit_contents_raw:
                    # text_raw에 별지 내용 추가
                    merged_text_raw = clause.text + "\n\n[별지 내용]\n" + "\n\n".join(exhibit_contents_raw)
                    
                    # text_norm에 별지 내용 추가 (// 구분자 사용)
                    merged_text_norm = clause.text_norm or clause.text
                    if exhibit_contents_norm:
                        merged_text_norm += "//" + "//".join(exhibit_contents_norm)
                    
                    # 새로운 ClauseData 생성 (병합된 내용으로)
                    merged_clause = ClauseData(
                        id=clause.id,
                        title=clause.title,
                        subtitle=clause.subtitle,
                        type=clause.type,
                        text=merged_text_raw,
                        text_norm=merged_text_norm,
                        embedding=clause.embedding  # 기존 임베딩 유지 (나중에 재생성 필요)
                    )
                    merged_clauses.append(merged_clause)
                    self.logger.info(f"✅ 조문 '{clause.id}'에 별지 참조 병합 완료")
                else:
                    # 별지를 찾지 못한 경우 원본 유지
                    merged_clauses.append(clause)
                    self.logger.warning(f"⚠️ 조문 '{clause.id}'의 별지 참조를 찾을 수 없음: {exhibit_refs}")
            else:
                # 별지 참조가 없으면 원본 유지
                merged_clauses.append(clause)
