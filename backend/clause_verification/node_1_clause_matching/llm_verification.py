...
try:
    from backend.clause_verification.node_1_clause_matching.models import ClauseData, VerificationDecision
...
class LLMVerificationService:
    ...
    # 프롬프트 템플릿
    VERIFICATION_PROMPT_TEMPLATE = """당신은 계약서 조문 비교 전문가입니다. 
두 개의 계약서 조문이 의미적으로 유사한지 판단해야 합니다.

**평가 원칙:**
1. **주제만 같다고 일치가 아닙니다.** 구체적인 내용과 법적 기능까지 유사해야 합니다.
2. **표준 조문의 핵심 내용이 사용자 조문에 실질적으로 포함되어 있는지** 확인하세요.

**중요 지침:**
1. 조문의 제목이나 번호는 무시하고, 오직 내용만 비교하세요.
2. 표준 조문은 상세한 활용안내문에서 발췌한 것이고, 사용자 조문은 간략한 실제 계약서입니다.
3. **주제가 같아도 구체적 내용이 다르면 불일치**입니다. 예:
   - 표준: "금융기관 거래정지 시 해지" ↔ 사용자: "지급 불이행 시 해지" → **불일치** (둘 다 해지지만 사유가 다름)
   - 표준: "영업취소 처분 시 해지" ↔ 사용자: "무단 이용 시 해지" → **불일치** (해지 사유가 완전히 다름)
4. **일치로 판단하는 경우**:
   - 표준: "데이터 제공 및 이용허락" ↔ 사용자: "데이터 제공" → 일치 (핵심 내용 포함)
   - 표준: "대가 지급 방법 및 기한" ↔ 사용자: "대가 지급" → 일치 (핵심 내용 포함)
   - 표준: "비밀유지 의무 및 예외" ↔ 사용자: "비밀유지 의무" → 일치 (핵심 내용 포함)
5. 사용자 조문이 표준 조문을 **간략히 표현**했거나 **일부 세부사항을 생략**한 경우는 일치입니다.
6. 하지만 **완전히 다른 내용**을 다루거나 **법적 기능이 다르면** 불일치입니다.
7. 의심스러울 경우 **불일치로 판단**하되, 근거를 명확히 제시하세요.
...
위 두 조문이 의미적으로 유사한지 판단하고, 다음 JSON 형식으로 응답하세요:

{{
    "is_match": true 또는 false,
    "confidence": 0.0에서 1.0 사이의 신뢰도 점수 (관대하게 평가),
    "reasoning": "판단 근거를 한국어로 자세히 설명"
}}
